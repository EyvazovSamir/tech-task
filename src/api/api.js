...
class Api {
  ...
  async getSomeAmountOfOrders(numberOfOrders, status) {
    let orders = [];
    try {
      const requestTokenServerResponse =
        await this._getRequestTokenServerResponse();
      const requestToken = requestTokenServerResponse.data.RequestToken;
      const accessTokenServerResponse =
        await this._getAccessTokenServerResponse(
          requestToken,
          this._privateKey,
          this._publicKey
        );
      const accessToken = accessTokenServerResponse.data.AccessToken;
      const ordersDataObject = await this._getOrdersDataObject(
        accessToken,
        numberOfOrders,
        status
      );
